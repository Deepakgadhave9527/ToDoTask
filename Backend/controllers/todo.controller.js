...
class TodoCtrl {
    static createTask(req, res) {
        ...
        taskDocument.save().then(result => {
            res.status(201).send({ message: "Task is created", result: taskDataFromClient })
    ...
    static updateTask(req, res) {
        const { id } = req.params;
        const taskDataFromClient = req.body;
        TodoModel.updateOne({ _id: id }, taskDataFromClient, { new: true }).then((result) => {
    ...
    static deleteTask(req, res) {
        const { id } = req.params;
        const taskDataFromClient = req.body;
        TodoModel.findOneAndDelete({ _id: id }).then(result => {
    ...
    static fetchSingleTask(req, res) {
        ...
        TodoModel.findOne({ _id: id }).then(result => {
            ...
            res.status(404).send({ message: "Single User is not is fetched", result: error })
        })
